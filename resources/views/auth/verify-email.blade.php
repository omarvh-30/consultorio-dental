...
<div class="min-h-screen grid grid-cols-1 lg:grid-cols-2 overflow-hidden relative">
    ...
    <div class="hidden lg:flex flex-col justify-center px-16
        ...
        <div class="max-w-lg z-30 relative">
            ...
            <p class="text-gray-700 text-lg leading-relaxed animate-slideInLeft delay-100">
                ...
                Si no recibiste el correo, podemos enviarte uno nuevo.
            </p>
        </div>
    ...
    {{-- PANEL DERECHO - MENSAJES Y BOTONES --}}
    <div class="flex items-center justify-center px-6 bg-gray-50 relative overflow-hidden">
        ...
        <div class="w-full max-w-md bg-white
            ...
            <div class="mb-4 text-sm text-gray-600 text-center animate-slideInUp delay-100">
                Gracias por registrarte. Antes de continuar, verifica tu correo haciendo clic en el enlace enviado.
                Si no recibiste el correo, puedes solicitar uno nuevo.
            </div>

            @if (session('status') == 'verification-link-sent')
                <div class="mb-4 font-medium text-sm text-green-600 text-center animate-slideInUp delay-200">
                    Se ha enviado un nuevo enlace de verificación a tu correo.
                </div>
            @endif

            <div class="mt-4 flex flex-col gap-4 animate-slideInUp delay-300">
                <form method="POST" action="{{ route('verification.send') }}">
                    @csrf
                    <x-primary-button class="w-full py-3 rounded-xl
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                            class="w-full py-3 rounded-xl border border-gray-300 text-gray-700 hover:bg-gray-100
                                   transition-transform transform hover:scale-105 duration-300 font-medium">
                        {{ __('Cerrar sesión') }}
                    </button>
                </form>
            </div>

            <div class="text-xs text-gray-400 mt-8 text-center animate-slideInUp delay-400">
